...
const genAI = new GoogleGenerativeAI(process.env.GEMINI_map_KEY);
...
app.post('/api/generate-trip', async (req, res) => {
    try {
        const { destination, language, duration, transport, budget } = req.body;
        
        // Create prompt for Gemini API
        const prompt = `Create a detailed trip plan for ${destination} for ${duration} days. 
        The plan should use ${transport} as the primary mode of transportation with a ${budget} budget.
